...
class SQLQueryLog(db.Model):
    """Log of SQL queries generated by the chatbot for review"""
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)
    conversation_id = db.Column(db.Integer, db.ForeignKey('chat_conversation.id'), nullable=True)
    question = db.Column(db.Text, nullable=False)  # User's original question
    generated_sql = db.Column(db.Text, nullable=False)  # Generated SQL query
    result_count = db.Column(db.Integer, nullable=True)  # Number of results returned
    executed_at = db.Column(db.DateTime, default=datetime.utcnow)
    error_message = db.Column(db.Text, nullable=True)  # Error if query failed
    ...
    def __repr__(self):
        return f'<SQLQueryLog {self.id} - User {self.user_id}>'
